...
# Scene Class
class Scene02CountableInfinity(VoiceoverScene):
    def construct(self):
        # Qwen cached voiceover (precache required)
        self.set_speech_service(get_speech_service(Path(__file__).resolve().parent))

        # Animation Sequence
        # Timing budget: 0.15 + 0.25 + 0.35 + 0.25 = 1.0 ✓
        with self.voiceover(text=SCRIPT["countable"]) as tracker:
            # Title
            title = Text("Countable Infinity", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.15)

            # Natural numbers display
            natural_nums = MathTex("1, 2, 3, \\dots", font_size=36)
            natural_nums.next_to(title, DOWN, buff=0.8)
            safe_position(natural_nums)
            play_text_in_slot(self, Write(natural_nums), tracker.duration * 0.25)

            # Animate counting with dots
            dots = VGroup()
            labels = VGroup()
            for i in range(1, 11):  # Show first 10 dots
                dot = Dot(radius=0.1, color=BLUE)
                dot.move_to(LEFT * 4 + RIGHT * (i - 1) * 0.8)
                dots.add(dot)
                label = Text(str(i), font_size=18, color=WHITE)
                label.next_to(dot, DOWN, buff=0.1)
                labels.add(label)
            dots_and_labels = VGroup(dots, labels).move_to(ORIGIN + DOWN * 1.5)
            safe_position(dots_and_labels)
            play_in_slot(self, *[Create(dot) for dot in dots], tracker.duration * 0.35)

            # Show bijection concept with aleph-zero
            bijection_text = Text("Bijection to Naturals", font_size=24, color=YELLOW)
            bijection_text.next_to(dots_and_labels, DOWN, buff=0.5)
            safe_position(bijection_text)
            aleph_zero = MathTex("\\aleph_0", font_size=48, color=GOLD)
            aleph_zero.next_to(bijection_text, DOWN, buff=0.3)
            safe_position(aleph_zero)
            play_in_slot(
                self, Write(bijection_text), Write(aleph_zero), tracker.duration * 0.25
            )
