...
# Scene Class
class Scene07Predictions(VoiceoverScene):
    def construct(self):
        # Qwen cached voiceover (precache required)
        self.set_speech_service(get_speech_service(Path(__file__).resolve().parent))

        # Animation Sequence
        # Timing budget: 0.2 + 0.8 = 1.0 ✓
        with self.voiceover(text=SCRIPT["predictions"]) as tracker:
            # Title
            title = Text("Falsifiable Predictions", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Predictions list
            pred1 = Text(
                "1. η scales inversely with k across taxa", font_size=28, color=BLUE
            )
            pred2 = Text(
                "2. Human tetrachromats: B ↑, η ≤ 16.7", font_size=28, color=GREEN
            )
            pred3 = Text(
                "3. Weak tetrachromats: B same, η halved", font_size=28, color=YELLOW
            )
            pred4 = Text(
                "4. Synthetic sensors: no fine discrimination gain beyond η < 1",
                font_size=28,
                color=RED,
            )

            predictions = VGroup(pred1, pred2, pred3, pred4).arrange(
                DOWN, buff=0.3, aligned_edge=LEFT
            )
            predictions.move_to(ORIGIN)
            safe_position(predictions)
            play_in_slot(self, Write(predictions), tracker.duration * 0.8)
