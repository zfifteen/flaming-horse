...
# Scene Class
class Scene05Properties(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["properties"]) as tracker:
            ...
            # Title
            title = Text("Properties and Applications", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_next(self, beats, Write(title))

            # Number line with irrationals
            number_line = NumberLine(
                x_range=[-2, 2, 1], length=6, include_numbers=True, label_direction=DOWN
            )
            number_line.move_to(ORIGIN)
            play_next(self, beats, Create(number_line))

            # Show density: mark some points
            pi_point = Dot(number_line.n2p(np.pi), color=GOLD)
            e_point = Dot(number_line.n2p(np.e), color=BLUE)
            sqrt2_point = Dot(number_line.n2p(np.sqrt(2)), color=GREEN)
            points = VGroup(pi_point, e_point, sqrt2_point)
            labels = VGroup(
                MathTex(r"\pi", font_size=24).next_to(pi_point, UP),
                MathTex(r"e", font_size=24).next_to(e_point, UP),
                MathTex(r"\sqrt{2}", font_size=24).next_to(sqrt2_point, UP),
            )
            safe_layout(labels)
            play_next(self, beats, FadeIn(points), FadeIn(labels))
