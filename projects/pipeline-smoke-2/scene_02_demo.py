...
# Scene Class
class Scene02Demo(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["demo"]) as tracker:
            # Main text
            main_text = Text("Rendering Process", font_size=48, weight=BOLD)
            main_text.move_to(UP * 3.8)
            play_text_in_slot(self, Write(main_text), tracker.duration * 0.4)

            # Bullet points
            bullet1 = Text("• Text Animation", font_size=32)
            bullet1.next_to(main_text, DOWN, buff=0.5)
            safe_position(bullet1)
            play_text_in_slot(self, FadeIn(bullet1), tracker.duration * 0.3)

            bullet2 = Text("• Voiceover Sync", font_size=32)
            bullet2.next_to(bullet1, DOWN, buff=0.3)
            safe_position(bullet2)
            play_text_in_slot(self, FadeIn(bullet2), tracker.duration * 0.3)
