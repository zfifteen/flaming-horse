...
# ── Scene Class ────────────────────────────────────────────────────
class Scene03ViscosityWeakness(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["viscosity_weakness"]) as tracker:
            # Title
            title = Text("Viscosity Paradox Exposed", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            self.play(Write(title), run_time=tracker.duration * 0.4)

            # Subtitle with safe positioning
            subtitle = Text("Beyond Turbulence Closure", font_size=32)
