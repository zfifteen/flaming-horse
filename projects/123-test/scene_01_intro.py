...
class Scene01Intro(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            # Title
            title = Text("The Fibonacci Sequence", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.25)

            # Subtitle
            subtitle = Text(
                "A Pattern in Nature and Mathematics", font_size=32, color=BLUE
            )
            subtitle.next_to(title, DOWN, buff=0.5)
            safe_position(subtitle)
            play_text_in_slot(self, FadeIn(subtitle), tracker.duration * 0.25)

            # Examples list
            examples = (
                VGroup(
                    Text("• Pinecone spirals", font_size=24),
                    Text("• Flower petals", font_size=24),
                    Text("• Leaf arrangements", font_size=24),
                    Text("• Galaxy structures", font_size=24),
                )
                .arrange(DOWN, aligned_edge=LEFT, buff=0.2)
                .move_to(ORIGIN)
            )
            safe_position(examples)
            play_in_slot(self, FadeIn(examples, lag_ratio=0.1), tracker.duration * 0.5)
