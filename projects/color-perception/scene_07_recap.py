...
# Scene Class
class Scene07Recap(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["recap"]) as tracker:
            # Title
            title = Text("Recap and Key Takeaways", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Bullet List
            bullet1 = Text("• More receptors ≠ richer perception", font_size=32)
            bullet1.move_to(LEFT * 3 + UP * 1)
            bullet2 = Text("• η = JNDs / (k(k-1)) measures efficiency", font_size=32)
            bullet2.next_to(bullet1, DOWN, buff=0.4)
            safe_position(bullet2)
            bullet3 = Text("• η inversely scales with k across species", font_size=32)
            bullet3.next_to(bullet2, DOWN, buff=0.4)
            safe_position(bullet3)
            play_in_slot(
                self,
                AnimationGroup(
                    FadeIn(bullet1), FadeIn(bullet2), FadeIn(bullet3), lag_ratio=0.2
                ),
