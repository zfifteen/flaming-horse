...
def check_scaffold_template():
    ...
    checks = [
        ...
        ("NO BeatPlan import", "BeatPlan", False),
        ("NO play_next import", "play_next", False),
        ("NO play_text_next import", "play_text_next", False),
    ]
...
def check_parser_functions():
    ...
    checks.append(
        ("build_scenes checks artifacts", "has_scaffold_artifacts", build_source)
    )
    ...
    checks.append(
        (
            "inject validates empty body",
            "at least one non-comment statement",
            inject_source,
        )
    )
    checks.append(
        (
            "inject handles indentation",
            "# Re-indent all lines to 12 spaces",
            inject_source,
        )
    )
...
def check_scene_helpers():
    ...
    # BeatPlan, play_next, play_text_next should NOT be present
    forbidden = [
        "BeatPlan",
        "play_next",
        "play_text_next",
    ...
    for export in expected_exports:
        ...
        defined = f"def {export}(" in content
    ...
    # Check forbidden exports are NOT present
    for export in forbidden:
        in_all = f"'{export}'" in content or f'"{export}"' in content
        status = "✅" if not in_all else "❌"
        print(f"  {status} {export} NOT exported")
        if in_all:
            all_passed = False
...
def run_quick_tests():
    ...
    try:
        # Test 1: Parse valid body code (without beats)
...
"""
        ...
        test1_passed = body is not None and "Write(title)" in body
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check no beat code in output
            test5_passed = "BeatPlan" not in full_code
            print(f"  {'✅' if test5_passed else '❌'} No beat code in output")
