...
def harmonious_color(base_color, variations=3, lightness_shift=0.1):
    ...
    for i in range(variations):
        h_shift = i / variations
        new_h = (h + h_shift) % 1
        new_l = min(1.0, l + lightness_shift * i)
        new_s = min(1.0, s)
...
def polished_fade_in(mobject, lag_ratio=0.2, scale_factor=1.1, glow=False):
    ...
    scale_anim = mobject.animate.scale(scale_factor)
    scale_anim.rate_func = there_and_back_with_pause
...
class Scene02Components(VoiceoverScene):
    def construct(self):
...
# Animation Sequence
        with self.voiceover(text=SCRIPT["components"]) as tracker:
            beats = BeatPlan(tracker.duration, [3, 2, 2, 2])
            
            # Palette already defined outside
            
            # Title
            title = Text("Key Components", font_size=48, weight=BOLD, color=blues[0])
            title = adaptive_title_position(title, None)
            play_text_next(self, beats, Write(title))
            
            # Subtitle
            subtitle = Text("Dividend, Divisor, Quotient, Remainder", font_size=32, color=blues[1])
            subtitle.next_to(title, DOWN, buff=0.4)
            safe_position(subtitle)
            play_text_next(self, beats, polished_fade_in(subtitle, lag_ratio=0.1))
            
            # Draw division bracket
            bracket_left = Line(UP * 1.5 + LEFT * 2, DOWN * 2 + LEFT * 2, stroke_width=12, color=blues[0])
            bracket_top = Line(LEFT * 2, RIGHT * 4, stroke_width=12, color=blues[0]).move_to(UP * 1.5)
            bracket = VGroup(bracket_left, bracket_top)
            play_next(self, beats, Create(bracket), rate_func=smooth, max_run_time=2.0)
            
            # Label divisor
            divisor = MathTex(r"23", font_size=48, color=blues[1])
            divisor.next_to(bracket_left, LEFT, buff=0.8, aligned_edge=DOWN)
            safe_position(divisor)
            divisor_label = Text("Divisor", font_size=28, color=blues[1]).next_to(divisor, DOWN, buff=0.1)
            safe_position(divisor_label)
            
            # Label dividend
            dividend = MathTex(r"12345", font_size=48, color=blues[2])
            dividend.next_to(bracket_left, RIGHT, buff=0.5, aligned_edge=LEFT)
            safe_position(dividend)
            dividend_label = Text("Dividend", font_size=28, color=blues[2]).next_to(dividend, RIGHT, buff=0.2)
            safe_position(dividend_label)
            
            # Label quotient
            quotient_label = Text("Quotient", font_size=28, color=blues[0]).next_to(bracket_top, UP, buff=0.3)
            safe_position(quotient_label)
            
            # Label remainder
            remainder_label = Text("Remainder", font_size=28, color=blues[2]).move_to(DOWN * 2 + RIGHT * 3)
            safe_position(remainder_label)
            
            # Layout validation
            labels_group = VGroup(divisor, divisor_label, dividend, dividend_label, quotient_label, remainder_label)
            safe_position(labels_group)
            
            play_next(self, beats, FadeIn(labels_group, lag_ratio=0.15))
        
        self.play(FadeOut(VGroup(title, subtitle, bracket, divisor, divisor_label, dividend, dividend_label, quotient_label, remainder_label)), run_time=1.0)