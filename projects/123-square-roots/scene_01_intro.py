...
class Scene01Intro(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            # Title
            title = Text("Understanding Square Roots", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.4)

            # Question
            question = Text("What number multiplied by itself gives 16?", font_size=32)
            question.next_to(title, DOWN, buff=0.5)
            safe_position(question)
            play_text_in_slot(self, FadeIn(question), tracker.duration * 0.6)
