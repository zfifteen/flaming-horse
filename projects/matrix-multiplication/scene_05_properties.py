...
class Scene05Properties(VoiceoverScene):
    def construct(self):
        ...
        # Animation Sequence
        # Timing budget: 0.25 + 0.35 + 0.4 = 1.0 ✓
        with self.voiceover(text=SCRIPT["properties"]) as tracker:
            # Title
            title = Text("Key Properties and Applications", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.25)

            # Non-commutativity equation
            non_comm = MathTex(r"AB \neq BA", font_size=60, color=RED)
            non_comm.next_to(title, DOWN, buff=0.8)
            safe_position(non_comm)
            play_in_slot(self, Write(non_comm), tracker.duration * 0.35)

            # Associativity equation and applications
            assoc = MathTex(r"(AB)C = A(BC)", font_size=60, color=GREEN)
            assoc.next_to(non_comm, DOWN, buff=0.8)
            safe_position(assoc)
            play_in_slot(self, Write(assoc), tracker.duration * 0.4)