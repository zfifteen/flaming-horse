...
class Scene04Results(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["results"]) as tracker:
            # SLOT_START:scene_body
            beats = BeatPlan(tracker.duration, [1, 1, 1, 1])

            palette = [BLUE, GREEN, ORANGE]

            title = Text("Results and Metrics", font_size=48, weight=BOLD, color=palette[0])
            title.move_to(UP * 3.8)
            play_text_next(self, beats, Write(title))

            subtitle = Text("Model Performance", font_size=36, color=palette[1])
            subtitle.next_to(title, DOWN, buff=0.4)
            safe_position(subtitle)
            play_text_next(self, beats, FadeIn(subtitle))

            r2_arc = Arc(radius=1.2, angle=0, stroke_width=20, stroke_color=palette[1]).set_start_angle(3*PI/4)
            r2_label = MathTex(r"R^2 = 0.85").scale(0.8).next_to(r2_arc, DOWN*0.5)
            r2_gauge = VGroup(r2_arc, r2_label)

            rmse_arc = Arc(radius=1.2, angle=0, stroke_width=20, stroke_color=palette[2]).set_start_angle(3*PI/4)
            rmse_label = Text("RMSE = 0.12").scale(0.8).next_to(rmse_arc, DOWN*0.5)
            rmse_gauge = VGroup(rmse_arc, rmse_label)

            gauges = VGroup(r2_gauge.move_to(LEFT * 1.5), rmse_gauge.move_to(RIGHT * 1.5))
            safe_position(gauges)

            header = VGroup(Text("Metric"), Text("Baseline"), Text("Model"), Text("Improv.")).arrange(RIGHT, buff=0.2).set_color(BLUE)
            row1 = VGroup(Text(r"R^2"), Text("0.72"), Text("0.85"), Text("+18%")).arrange(RIGHT, buff=0.2)
            row2 = VGroup(Text("RMSE"), Text("0.17"), Text("0.12"), Text("-29%")).arrange(RIGHT, buff=0.2)
            comp_table = VGroup(header, row1, row2).arrange(DOWN, buff=0.15).scale(0.9).move_to(DOWN * 1.0)

            safe_position(comp_table)
            safe_layout(*gauges, comp_table)

            play_next(self, beats, LaggedStart(r2_arc.animate.set_angle(0.7 * PI), rmse_arc.animate.set_angle(0.4 * PI), FadeIn(VGroup(r2_label, rmse_label))))

            play_next(self, beats, LaggedStart(*(FadeIn(row) for row in [header, row1, row2])))

            improv_labels = VGroup(header[3], row1[3], row2[3])
            play_next(self, beats, ShowPassingFlash(improv_labels.set_color(GREEN)))
