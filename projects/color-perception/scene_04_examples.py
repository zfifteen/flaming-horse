...
class Scene04Examples(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["examples"]) as tracker:
            # Title
            title = Text("Cross-Species Examples", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Species Data
            human = Text("Humans: k=3, η=25", font_size=32)
            human.move_to(LEFT * 4 + UP * 1)
            mantis = Text("Mantis Shrimp: k=12, η=0.09", font_size=32)
            mantis.move_to(ORIGIN + UP * 1)
            tetra = Text("Tetrachromats: k=4, η=?", font_size=32)
            tetra.move_to(RIGHT * 4 + UP * 1)
            safe_layout(human, mantis, tetra)
            play_in_slot(
                self,
                AnimationGroup(
                    FadeIn(human), FadeIn(mantis), FadeIn(tetra), lag_ratio=0.2
                ),
