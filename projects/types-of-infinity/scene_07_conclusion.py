...
class Scene07Conclusion(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["conclusion"]) as tracker:
            # Title
            title = Text("Conclusion: The Many Infinities", font_size=48, weight=BOLD)
            ...
            bullets = [
                ...
                "Uncountable Infinity (Continuum)",
                "Cardinal Numbers",
                "Ordinal Numbers",
            ...
            for i, bullet in enumerate(bullets):
                text = Text(f"• {bullet}", font_size=32)
                text.move_to(UP * (2.5 - i * 0.8))
                bullet_texts.append(text)

            safe_layout(*bullet_texts)

            # Fade in bullets sequentially
            for i, bullet_text in enumerate(bullet_texts):
                fraction = 0.12  # 0.12 * 5 = 0.6
                play_text_in_slot(
                    self, FadeIn(bullet_text), tracker.duration * fraction
                )

            # Final infinity symbol
            infinity_symbol = MathTex(r"\infty", font_size=72, color=GOLD)
            infinity_symbol.move_to(ORIGIN)
            play_in_slot(self, FadeIn(infinity_symbol), tracker.duration * 0.08)
