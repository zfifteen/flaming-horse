...
# Scene Class
class Scene03Recap(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["recap"]) as tracker:
            # Recap title
            title = Text("Summary", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.4)

            # Summary text
            summary = Text("Pipeline Verified Successfully", font_size=32)
            summary.next_to(title, DOWN, buff=0.5)
            safe_position(summary)
            play_text_in_slot(self, FadeIn(summary), tracker.duration * 0.3)
