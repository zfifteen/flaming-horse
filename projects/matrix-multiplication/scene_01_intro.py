...
class Scene01Intro(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            # Title
            title = Text("Matrix Multiplication", font_size=48, weight=BOLD)
            ...
            safe_position(matrices)
            play_in_slot(self, FadeIn(matrices), tracker.duration * 0.4)

            # Multiplication symbol
            times = MathTex(r"\times", font_size=48, color=YELLOW)
            times.move_to(LEFT * 2.5)
            play_in_slot(self, FadeIn(times), tracker.duration * 0.3)
