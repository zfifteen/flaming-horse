...
class Scene01Intro(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            ...
            title = Text("Types of Mathematical Infinity", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.4)

            # Infinite symbols
            infinity_symbol = MathTex(r"\infty", font_size=72, color=BLUE)
            infinity_symbol.move_to(ORIGIN)
            play_in_slot(self, FadeIn(infinity_symbol), tracker.duration * 0.3)

            # Introductory graphic
            graphic = Circle(radius=1.5, color=GREEN, fill_opacity=0.2)
            graphic.move_to(DOWN * 1.5)
            play_in_slot(self, Create(graphic), tracker.duration * 0.3)
