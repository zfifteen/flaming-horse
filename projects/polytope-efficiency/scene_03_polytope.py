...
# Scene Class
class Scene03Polytope(VoiceoverScene):
    def construct(self):
        # Qwen cached voiceover (precache required)
        self.set_speech_service(get_speech_service(Path(__file__).resolve().parent))

        # Animation Sequence
        # Timing budget: 0.15 + 0.25 + 0.3 + 0.3 = 1.0 ✓
        with self.voiceover(text=SCRIPT["polytope"]) as tracker:
            # Title
            title = Text("The Markov Polytope Framework", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.15)

            # Polytope definition text
            definition = Text(
                "A convex polytope representing\nall possible qualia kernels",
                font_size=32,
                color=BLUE,
            )
            ...
            # Dimension formula
            formula = MathTex(r"D = n(n-1)", font_size=48, color=GOLD)
            formula.move_to(ORIGIN)
            play_in_slot(self, Write(formula), tracker.duration * 0.3)

            # Illustrate with simple geometric shape (tetrahedron vertices)
            tetrahedron = Polygon(
                np.array([-1, -1, 0]),
                np.array([1, -1, 0]),
                np.array([0, 1, 0]),
            ...
            tetrahedron.move_to(DOWN * 1.5)
            play_in_slot(self, Create(tetrahedron), tracker.duration * 0.3)
