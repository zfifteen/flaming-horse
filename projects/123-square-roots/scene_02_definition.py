...
class Scene02Definition(VoiceoverScene):
    def construct(self):
        ...
        # 0.15 + 0.25 + 0.2 + 0.2 + 0.2 = 1.0 ✓
        with self.voiceover(text=SCRIPT["definition"]) as tracker:
            # Title
            title = Text("What is a Square Root?", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.15)

            # Definition text
            definition = Text(
                "A square root is a value that, when multiplied by itself,\nproduces the original number.",
                font_size=24,
                line_spacing=1.2,
            )
            definition.next_to(title, DOWN, buff=0.8)
            safe_position(definition)
            play_text_in_slot(self, FadeIn(definition), tracker.duration * 0.25)

            # Example equation: √9 = 3
            example_eq = MathTex(r"\sqrt{9} = 3", font_size=48, color=GOLD)
            example_eq.next_to(definition, DOWN, buff=0.8)
            safe_position(example_eq)
            play_in_slot(self, Write(example_eq), tracker.duration * 0.2)

            # Multiplication: 3 × 3 = 9
            multiplication = MathTex(r"3 \times 3 = 9", font_size=36, color=BLUE)
            multiplication.next_to(example_eq, DOWN, buff=0.5)
            safe_position(multiplication)
            play_in_slot(self, Write(multiplication), tracker.duration * 0.2)

            # Note about positive/negative roots and zero
            note = Text(
                "Every positive number has two square roots: +3 and -3\n(except zero, which has just one: 0)",
                font_size=20,
                color=GREEN,
            )
            note.next_to(multiplication, DOWN, buff=0.5)
            safe_position(note)
            play_text_in_slot(self, FadeIn(note), tracker.duration * 0.2)
