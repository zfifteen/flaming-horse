...
# Scene Class
class Scene05OrdinalInfinities(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["ordinals"]) as tracker:
            # Title: Ordinal Infinities
            title = Text("Ordinal Infinities", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.15)

            # Cardinals vs Ordinals comparison
            cardinals_label = Text("Cardinals:", font_size=32, color=BLUE)
            cardinals_label.move_to(LEFT * 3 + UP * 1.5)
            cardinals_desc = Text("Measure size", font_size=24, color=BLUE)
            cardinals_desc.next_to(cardinals_label, DOWN, buff=0.2)
            safe_position(cardinals_desc)

            ordinals_label = Text("Ordinals:", font_size=32, color=GREEN)
            ordinals_label.move_to(RIGHT * 3 + UP * 1.5)
            ordinals_desc = Text("Capture order", font_size=24, color=GREEN)
            ordinals_desc.next_to(ordinals_label, DOWN, buff=0.2)
            safe_position(ordinals_desc)

            play_text_in_slot(self, Write(cardinals_label), tracker.duration * 0.125)
            play_text_in_slot(self, Write(cardinals_desc), tracker.duration * 0.125)

            # Omega symbol
            omega = MathTex(r"\omega", font_size=72, color=GOLD)
            omega.move_to(ORIGIN + UP * 0.5)
            play_in_slot(self, Write(omega), tracker.duration * 0.2)

            # Ordinal sequence
            ordinal_seq = MathTex(
                r"1 < 2 < 3 < \cdots < \omega < \omega + 1 < \omega \cdot 2 < \cdots",
                font_size=36,
            )
            ordinal_seq.move_to(DOWN * 1.5)
            play_in_slot(self, Write(ordinal_seq), tracker.duration * 0.4)
