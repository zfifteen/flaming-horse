...
class Scene06Recap(VoiceoverScene):
    def construct(self):
        ...
        # Animation Sequence
        # Timing budget: 0.25 + 0.5 + 0.25 = 1.0 ✓
        with self.voiceover(text=SCRIPT["recap"]) as tracker:
            # Section title
            title = Text("Summary and Key Takeaways", font_size=36, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.25)

            # Key points
            key_points = VGroup(
                Text("• Square root: number × itself = original", font_size=24),
                Text("• Exact for perfect squares (4, 9, 16, 25, 36)", font_size=24),
                Text("• Approximations for non-perfect squares", font_size=24),
                Text("• Geometric interpretation: side length of square", font_size=24),
                Text("• Applications in physics, finance, engineering", font_size=24),
            )
            key_points.arrange(DOWN, buff=0.4, aligned_edge=LEFT)
            key_points.move_to(ORIGIN)
            safe_layout(key_points)
            play_in_slot(
                self,
                *[FadeIn(point, lag_ratio=0.2) for point in key_points],
                tracker.duration * 0.5,
            )

            # Fade out
            self.play(FadeOut(key_points), run_time=tracker.duration * 0.25)
