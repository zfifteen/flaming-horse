...
# Scene Class
class Scene04Visualization(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["visualization"]) as tracker:
            # Title
            title = Text("Geometric Interpretation", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Vector and matrix
            vector = Arrow(start=ORIGIN, end=RIGHT * 2 + UP * 1, buff=0, color=BLUE)
            vector_label = MathTex(r"\vec{v}", font_size=24).next_to(
                vector.get_end(), RIGHT
            )
            vector_group = VGroup(vector, vector_label)
            vector_group.move_to(LEFT * 3)
            safe_position(vector_group)

            matrix = Matrix([["2", "0"], ["0", "1"]])
            matrix.move_to(RIGHT * 3)
            safe_position(matrix)

            self.play(
                Create(vector_group), Write(matrix), run_time=tracker.duration * 0.3
            )

            # Transformation
            transformed_vector = Arrow(
                start=ORIGIN, end=RIGHT * 4 + UP * 1, buff=0, color=RED
            )
            transformed_label = MathTex(r"A\vec{v}", font_size=24).next_to(
                transformed_vector.get_end(), RIGHT
            )
            transformed_group = VGroup(transformed_vector, transformed_label)
            transformed_group.move_to(LEFT * 3)
            safe_position(transformed_group)

            self.play(
                Transform(vector_group, transformed_group),
                run_time=tracker.duration * 0.3,
            )

            # Grid deformation hint
            grid = NumberPlane(
                x_range=(-2, 4, 1),
                y_range=(-2, 3, 1),
                background_line_style={"stroke_color": GRAY, "stroke_width": 1},
            )
            self.play(Create(grid), run_time=tracker.duration * 0.2)