...
# ── Scene Class ────────────────────────────────────────────────────
class Scene03HypothesisCore(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["hypothesis"]) as tracker:
            # Title (ALWAYS use UP * 3.8, NEVER .to_edge(UP))
            title = Text("The Hypothesis", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            self.play(Write(title), run_time=tracker.duration * 0.4)

            # Subtitle with safe positioning
            subtitle = Text("Immune Threshold Framework", font_size=32)
            ...
            # Main content
            formula = MathTex(
                r"I(P) = \begin{cases} 0 & P < T \\ 1 & P \geq T \end{cases}",
                font_size=48,
                color=BLUE,
            )
