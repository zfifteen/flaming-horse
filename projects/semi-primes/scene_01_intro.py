...
class Scene01Intro(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            ...
            # Title
            title = Text("What are Semi-Primes?", font_size=48, weight=BOLD)
            ...
            primes = VGroup(
                ...
                MathTex(r"3", font_size=40),
                MathTex(r"5", font_size=40),
                MathTex(r"7", font_size=40),
                MathTex(r"11", font_size=40),
            )
            primes.arrange(RIGHT, buff=0.4).move_to(UP * 1.2 + LEFT * 0.5)
            safe_layout(*primes)
            play_next(self, beats, *[FadeIn(p) for p in primes])

            # Multiplication example
            example = MathTex(r"3 \\times 5 = 15", font_size=48, color=BLUE)
            example.move_to(ORIGIN)
            play_next(self, beats, Write(example))
