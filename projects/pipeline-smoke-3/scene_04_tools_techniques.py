...
# Scene Class
class Scene04ToolsTechniques(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["tools_techniques"]) as tracker:
            # Title
            title = Text("Tools and Techniques", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.15)

            # List of tips
            tips = [
                "Use git commit -m for simple changes",
                "Omit -m to open editor for longer messages",
                "Tools like commitizen guide structured prompts",
                "Git hooks enforce message standards",
                "Reference issue numbers like 'Fix #123'",
                "Review message before committing",
            ]

            tip_texts = []
            for i, tip in enumerate(tips):
                tip_text = Text(f"• {tip}", font_size=28, color=BLUE)
                tip_text.move_to(UP * (2.5 - i * 0.6))
                tip_texts.append(tip_text)

            safe_layout(*tip_texts)

            # Animate tips with lag
            play_in_slot(
                self,
                *[FadeIn(tip) for tip in tip_texts],
                tracker.duration * 0.4,
                lag_ratio=0.1,
            )
            ...
            example1.next_to(tip_texts[0], RIGHT, buff=1.0)
            safe_position(example1)
            play_text_in_slot(self, Write(example1), tracker.duration * 0.15)

            example2 = Text(
                "git commit  # opens editor", font_size=24, color=GREEN, font="Courier"
            )
            example2.next_to(tip_texts[1], RIGHT, buff=1.0)
            safe_position(example2)
            play_text_in_slot(self, Write(example2), tracker.duration * 0.15)

            example3 = Text(
                "Fix #123: handle null user case",
                font_size=24,
                color=GREEN,
                font="Courier",
            )
            example3.next_to(tip_texts[4], RIGHT, buff=1.0)
            safe_position(example3)
            play_text_in_slot(self, Write(example3), tracker.duration * 0.15)
