...
class Scene04Applications(VoiceoverScene):
    def construct(self):
        ...
        # Animation Sequence
        # Timing budget: 0.1 + 0.25 + 0.15 + 0.2 + 0.15 + 0.15 = 1.0 ✓
        with self.voiceover(text=SCRIPT["applications"]) as tracker:
            ...
            self.play(FadeIn(table), run_time=tracker.duration * 0.25)

            # Transition to nature spirals
            spirals_text = Text(
                "Pinecones and pineapples\nshow spiral patterns\nbased on Fibonacci numbers",
                font_size=24,
            )
            spirals_text.move_to(ORIGIN)
            self.play(
                FadeOut(table), FadeIn(spirals_text), run_time=tracker.duration * 0.15
            )
            ...
            art_text.move_to(ORIGIN)
            self.play(
                FadeOut(spirals_text), FadeIn(art_text), run_time=tracker.duration * 0.2
            )

            # Transition to technology
            tech_text = Text(
                "Technology:\nComputer algorithms,\ndata structures", font_size=24
            )
            tech_text.move_to(ORIGIN)
            self.play(
                FadeOut(art_text), FadeIn(tech_text), run_time=tracker.duration * 0.15
            )

            # Conclusion
            conclusion_text = Text(
                "Bridging mathematics\nwith the real world", font_size=32, color=GOLD
            )
