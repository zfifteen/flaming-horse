...
def harmonious_color(base_color, variations=3, lightness_shift=0.1):
    rgb = color_to_rgb(base_color); r, g, b = colorsys.rgb_to_hls(*rgb[:3])
    palette = []
...
class Scene06Recap(VoiceoverScene):
    def construct(self):
        ...
        greens = harmonious_color(GREEN, variations=4)
        ...
        with self.voiceover(text=SCRIPT["recap"]) as tracker:
            beats = BeatPlan(tracker.duration, [2, 2, 2, 2, 2, 2])
            
            # Title (ALWAYS use UP * 3.8, NEVER .to_edge(UP)); Adaptive (New)
            title = Text("Recap and Key Takeaways", font_size=48, weight=BOLD, color=greens[0])
            ...
            subtitle = Text("Polynomial Long Division", font_size=32, color=greens[1])
            ...
            bullets = VGroup(
                Text("• Divide leading terms", font_size=28, color=greens[2]),
                Text("• Multiply across divisor", font_size=28, color=greens[2]),
                Text("• Subtract result", font_size=28, color=greens[2]),
                Text("• Bring down, repeat", font_size=28, color=greens[2]),
                Text("• Quotient + remainder", font_size=28, color=greens[2])
            ).arrange(DOWN, buff=0.25, aligned_edge=LEFT).move_to(ORIGIN)
            for bullet in bullets:
                safe_position(bullet)
            safe_layout(*bullets)
            play_next(self, beats, FadeIn(bullets, lag_ratio=0.3, run_time=2.5))
            
            # Pitfalls and CTA
            pitfalls = Text("Pitfalls: Signs, alignment, forget bring down!", font_size=28, color=YELLOW).move_to(DOWN * 2.0)
            safe_position(pitfalls)
            cta = Text("Practice now!", font_size=36, weight=BOLD, color=greens[3]).move_to(DOWN * 0.5)
            safe_position(cta)
            play_next(self, beats, polished_fade_in(pitfalls), polished_fade_in(cta))
            
            # Recap example: fade in full result, then fade out
            recap_example = VGroup(
                MathTex(r"x^2 + 2x - 1", font_size=36, color=GREEN),
                MathTex(r"R = -2", font_size=36, color=ORANGE)
            ).arrange(RIGHT, buff=1).move_to(UP * 1.5)
            safe_layout(*recap_example)
            play_next(self, beats, FadeIn(recap_example))
            play_next(self, beats, FadeOut(recap_example), max_run_time=1.0)
            
            # Ensure safe layout for all
            all_elements = VGroup(title, subtitle, bullets, pitfalls, cta)
            safe_layout(*all_elements)
