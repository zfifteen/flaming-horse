...
**Timeline:** Target completion in 1-2 days for prompt/template updates; verification via test builds.
...
- **Task 1.1:** Add "Bullet Content Rule" section under CRITICAL RULES.
  ...
  - Content: Prohibit using `narrative_beats` or `visual_ideas` as on-screen text; mandate derivation from `narration_script.py`; cap bullets at 30 characters or 6 words; forbid stage directions. Add examples (e.g., "For math: derive 'a² + b² = c²' from narration, not 'Explain the theorem.'").
...
- **Task 1.3:** Revise "Timing Budget" rules.
  ...
  - Add prohibition on double-consuming slots. Address `self.wait(remaining)` by filling remaining time with subtle animations (e.g., Indicate, color pulse) or remove text cap if 1.8s slots allow.
- **Task 1.4:** Update "Complete Scene Template".
  - Replace single rigid template with 2-3 structurally different examples (e.g., bullets, timeline, central quote reveal). Mandate unique flows, inject `topic_visual_patterns.md` content into agent context, require ≥1 unique visual per scene.
  - Adjust positions to `LEFT * 3.5`; use `set_max_width(6.0)`.
- **Task 1.5:** Audit and update `scripts/scaffold_scene.py`.
  - Confirm it does not emit hardcoded bullet patterns at `LEFT * 4.8` or `{{KEY_POINT_1}}` placeholders; update if necessary to align with new templates.
- **Dependencies:** None.
...
- **Task 2.3:** Centralize helper functions into a shared module (e.g., `flaming_horse_helpers.py`) to prevent inter-scene divergence.
- **Dependencies:** Parallel to Phase 1 (no dependency on prose updates).
...
- **Task 4.1:** Create `tests/test_scene_content.py` with functions: `test_no_planning_text_in_scenes`, `test_horizontal_bounds`.
  - Implement logic as per analysis (JSON parsing, regex scanning). Add test for `beats.next_slot()` isolation (fails if appears outside `play_next`/`play_text_next`).
- **Task 4.2:** Merge containment into Phase 4: Create standalone script `scripts/validate_scene_content.py` that runs pytest as a build gate.
...
- **Post-Implementation:** Run two test builds: one dense-text topic (e.g., Matrix simulation explainer for overflow stress), one math topic (e.g., "Pythagorean Theorem"). Verify generated scenes: no planning text, elements within bounds, minimal dead air (no micro-stuttering), unique structures.
- **Regression Runs:** Execute new tests and gate script on existing/future projects.
- **Metrics:** Check build.log for no overflow warnings; scene_qc_report.md should flag issues pre-render; pytest passes as gate.
...
- **Risk:** Agents misinterpret new directives.
  - Mitigation: Add explicit examples in AGENTS.md (e.g., "Example: For math topic, derive 'a² + b² = c²' from narration, not 'Explain the theorem.'"); ensure `topic_visual_patterns.md` is injected into context.
...
2. Commit changes incrementally with descriptive messages (e.g., "Fix bullet content rule in AGENTS.md").
3. Schedule test build after Phase 4 completion.</content>
