...
# Scene Class
class Scene02Problem(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["problem"]) as tracker:
            # Title: The Color Dimension Problem
            title = Text("The Color Dimension Problem", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Comparison Table: Humans vs Mantis Shrimp
            table_data = [
                ["Species", "Receptors", "Hue Steps"],
                ["Human", "3", "~130"],
                ["Mantis Shrimp", "12", "~12"],
            ]
            table = Table(
            ...
            safe_position(table)
            play_in_slot(self, Create(table), tracker.duration * 0.3)

            # Dichotomy Text
            dichotomy_title = Text("Processing Dichotomy", font_size=36, color=YELLOW)
            dichotomy_title.next_to(table, DOWN, buff=0.5)
            safe_position(dichotomy_title)
            play_text_in_slot(self, Write(dichotomy_title), tracker.duration * 0.2)

            # Opponent Processing (Humans)
            opponent = Text("Opponent Processing (Humans)", font_size=24, color=BLUE)
            opponent.next_to(dichotomy_title, DOWN, buff=0.3)
            safe_position(opponent)
            play_text_in_slot(self, FadeIn(opponent), tracker.duration * 0.15)

            # Interval Decoding (Mantis Shrimp)
            interval = Text(
                "Interval Decoding (Mantis Shrimp)", font_size=24, color=GREEN
            )
            interval.next_to(opponent, DOWN, buff=0.3)
            safe_position(interval)
            play_text_in_slot(self, FadeIn(interval), tracker.duration * 0.15)
