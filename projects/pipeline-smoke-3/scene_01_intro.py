...
class Scene01Intro(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            ...
            title = Text("Writing Great Git Commit Messages", font_size=48, weight=BOLD)
            ...
            # Git branch graphic
            branch_text = Text("Git Branch", font_size=32, color=BLUE)
            branch_text.next_to(title, DOWN, buff=1.0)
            safe_position(branch_text)  # ALWAYS call after .next_to()
            # Simple graphic: arrow from main to branch
            arrow = Arrow(start=LEFT * 2, end=RIGHT * 2, color=GREEN)
            arrow.next_to(branch_text, DOWN, buff=0.5)
            safe_position(arrow)
            graphic_group = VGroup(branch_text, arrow)
            slot = tracker.duration * 0.6
            self.play(FadeIn(graphic_group), run_time=min(2.0, max(0.3, slot)))
            self.wait(max(0.0, slot - min(2.0, max(0.3, slot))))
