...
class Scene05Conclusion(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["conclusion"]) as tracker:
            ...
            # Subtitle
            subtitle = Text("Key Takeaways", font_size=32)
            ...
            # Recap points
            recap = BulletedList(
                "Semi-primes are products of exactly two primes",
                "Bridge between primes and composites",
                "Important in number theory and cryptography",
                "Trial division for identification",
                "Various factorization methods available",
                font_size=24,
            ).scale(0.8)
            recap.next_to(subtitle, DOWN, buff=0.5)
            safe_position(recap)
            slot = tracker.duration * 0.55
            self.play(FadeIn(recap), run_time=min(2.0, max(0.3, slot)))
            self.wait(max(0.0, slot - min(2.0, max(0.3, slot))))
