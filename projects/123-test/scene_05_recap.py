...
# Scene Class
class Scene05Recap(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["recap"]) as tracker:
            ...
            # Title
            title = Text("Summary and Recap", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Summary points
            points = [
                "• Starts with 0 and 1, each number is sum of previous two",
                "• Connects to the golden ratio",
                "• Appears in nature: flower petals, spiral galaxies",
                "• Intriguing mathematical properties and applications",
                "• Reveals hidden order in the universe",
            ]
            point_texts = [Text(p, font_size=24) for p in points]

            # Position points vertically
            for i, pt in enumerate(point_texts):
                pt.move_to(UP * (2 - i * 0.8))
                safe_position(pt)

            # Fade in all points with lag
            play_in_slot(
                self,
                LaggedStart(*[FadeIn(pt) for pt in point_texts], lag_ratio=0.2),
                tracker.duration * 0.8,
            )
