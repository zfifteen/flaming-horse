...
class Scene01Intro(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            ...
            title = Text("Squaring the Circle", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.25)

            # Show circle on left, square on right
            ...
            play_in_slot(
                self,
                LaggedStart(Create(circle), Create(square), lag_ratio=0.2),
                tracker.duration * 0.4,
            )

            # Fade in problem statement
            problem_text = Text(
                "Can you construct a square with the same area\nas this circle using only compass and straightedge?",
                font_size=24,
                color=YELLOW,
            )
            problem_text.next_to(title, DOWN, buff=0.8)
            safe_position(problem_text)
            play_text_in_slot(self, FadeIn(problem_text), tracker.duration * 0.35)
