...
class Scene05Recap(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["recap"]) as tracker:
            ...
            bullets = VGroup(
                Text("• Clear: Be specific about what changed", font_size=24),
                Text("• Concise: Avoid unnecessary details", font_size=24),
                Text("• Reviewable: Provide enough context", font_size=24),
            )
            bullets.arrange(DOWN, aligned_edge=LEFT, buff=0.3)
            bullets.next_to(title, DOWN, buff=0.8)
            safe_position(bullets)
            play_text_in_slot(
                self, FadeIn(bullets, lag_ratio=0.3), tracker.duration * 0.6
            )

            # Final call-to-action
            cta = Text("Write better commits today!", font_size=36, color=YELLOW)
            cta.next_to(bullets, DOWN, buff=0.5)
            safe_position(cta)
            play_text_in_slot(self, Write(cta), tracker.duration * 0.2)
