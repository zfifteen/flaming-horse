...
# Scene Class
class Scene04CardinalNumbers(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["cardinals"]) as tracker:
            # Timing budget: 0.2 + 0.2 + 0.3 + 0.3 = 1.0

            # Title
            title = Text("Cardinal Numbers", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Aleph-zero
            aleph0 = MathTex(r"\aleph_0", font_size=60, color=BLUE)
            aleph0.move_to(UP * 1.5)
            label0 = Text("Size of Countable Sets", font_size=32, color=BLUE)
            label0.next_to(aleph0, DOWN, buff=0.5)
            safe_position(label0)
            play_in_slot(
                self, LaggedStart(Write(aleph0), FadeIn(label0)), tracker.duration * 0.2
            )

            # Aleph-one and continuum
            aleph1 = MathTex(r"\aleph_1", font_size=60, color=GREEN)
            aleph1.next_to(aleph0, RIGHT, buff=2.0)
            safe_position(aleph1)
            label1 = Text("Continuum", font_size=32, color=GREEN)
            label1.next_to(aleph1, DOWN, buff=0.5)
            safe_position(label1)
            continuum = MathTex(r"2^{\aleph_0}", font_size=48, color=YELLOW)
            continuum.next_to(label1, DOWN, buff=0.5)
            safe_position(continuum)
            ch_question = Text("Continuum Hypothesis?", font_size=24, color=RED)
            ch_question.next_to(continuum, DOWN, buff=0.3)
            safe_position(ch_question)
            play_in_slot(
                self,
                LaggedStart(
                    Write(aleph1), FadeIn(label1), Write(continuum), FadeIn(ch_question)
                ),
                tracker.duration * 0.3,
            )

            # Cardinal arithmetic
            arith1 = MathTex(
                r"\aleph_0 + \aleph_0 = \aleph_0", font_size=36, color=PURPLE
            )
            arith1.move_to(DOWN * 1.0)
            arith2 = MathTex(
                r"\aleph_0 \times \aleph_0 = \aleph_0", font_size=36, color=PURPLE
            )
            arith2.next_to(arith1, DOWN, buff=0.4)
            safe_position(arith2)
            arith3 = MathTex(r"2^{\aleph_0} > \aleph_0", font_size=36, color=PURPLE)
            arith3.next_to(arith2, DOWN, buff=0.4)
            safe_position(arith3)
            play_in_slot(
                self,
                LaggedStart(Write(arith1), Write(arith2), Write(arith3), lag_ratio=0.2),
                tracker.duration * 0.3,
            )
            #
