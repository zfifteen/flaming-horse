...
# Scene Class
class Scene02Examples(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["examples"]) as tracker:
            beats = BeatPlan(tracker.duration, [4, 3, 4, 3])

            # Title
            title = Text("Examples and Properties", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_next(self, beats, Write(title))

            # Small semi-primes (number examples)
            small1 = MathTex(r"4 = 2 \\times 2", font_size=36, color=ORANGE)
            small2 = MathTex(r"9 = 3^2", font_size=36, color=GREEN)
            small3 = MathTex(r"15 = 3 \\times 5", font_size=36, color=BLUE)
            small4 = MathTex(r"25 = 5^2", font_size=36, color=PURPLE)
            small_ex = (
                VGroup(small1, small2, small3, small4)
                .arrange(DOWN, buff=0.5)
                .move_to(LEFT * 4)
            )
            safe_position(small_ex)
            play_next(
                self, beats, LaggedStart(*[Write(s) for s in small_ex], lag_ratio=0.2)
            )

            # Properties and large
            prop1 = Text("Only even: 4", font_size=28, color=YELLOW)
            prop1.next_to(small_ex, RIGHT, buff=1.2)
            safe_position(prop1)

            prop2 = Text("Others odd", font_size=28, color=YELLOW).next_to(
                prop1, DOWN, buff=0.4
            )
            safe_position(prop2)

            prop3 = Text("Density <100: ~35%", font_size=28, color=YELLOW).next_to(
                prop2, DOWN, buff=0.4
            )
            safe_position(prop3)

            large = MathTex(r"143=11\\times13", font_size=32, color=BLUE).next_to(
                prop3, DOWN, buff=0.5
            )
            safe_position(large)

            props = VGroup(prop1, prop2, prop3, large)
            play_text_next(
                self,
                beats,
                Succession(
                    FadeOut(small_ex),
                    LaggedStart(*[FadeIn(p) for p in props], lag_ratio=0.15),
                ),
            )

            # Density graph
            def density(x):
                return 0.35 * np.log(100) / np.log(x + np.e)
            ...
            curve = axes.plot(density, color=RED, x_range=[10, 1000])

            label = Text("Density decreases", font_size=24, color=RED).next_to(
                curve, DOWN, buff=0.3
            )
            safe_position(label)
            ...
            play_next(
                ...
                Succession(
                    FadeOut(props),
                    LaggedStart(
                        Create(axes), Write(curve), FadeIn(label), lag_ratio=0.2
                    ),
                ),
            )
