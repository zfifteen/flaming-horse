...
# Scene Class
class Scene06Implications(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["implications"]) as tracker:
            # Title
            title = Text("Implications for Consciousness", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Decision Rule
            rule = Text(
                "Decision Rule: If η < 1, processing depth > sensor count",
                font_size=36,
                color=GREEN,
            )
            rule.next_to(title, DOWN, buff=0.5)
            safe_position(rule)
            play_text_in_slot(self, Write(rule), tracker.duration * 0.3)

            # Broader Implications
            ai = Text("For AI: Scale processing before adding sensors", font_size=32)
            ai.move_to(DOWN * 1)
            consciousness = Text(
                "For Consciousness: Tests Hoffman's polytope framework", font_size=32
            )
            consciousness.next_to(ai, DOWN, buff=0.3)
            safe_position(consciousness)
            play_in_slot(
                self,
                AnimationGroup(FadeIn(ai), FadeIn(consciousness), lag_ratio=0.3),
