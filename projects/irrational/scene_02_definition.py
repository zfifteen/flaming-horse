...
class Scene02Definition(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["definition"]) as tracker:
            ...
            # Title
            title = Text("What Are Irrational Numbers?", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_next(self, beats, Write(title))

            # Show fraction examples
            frac1 = MathTex(r"\frac{1}{2}", font_size=36)
            frac2 = MathTex(r"\frac{3}{4}", font_size=36)
            frac3 = MathTex(r"\frac{5}{8}", font_size=36)
            fractions = VGroup(frac1, frac2, frac3).arrange(RIGHT, buff=1.0)
            fractions.move_to(ORIGIN)
            safe_layout(frac1, frac2, frac3)
            play_next(self, beats, *[Write(f) for f in [frac1, frac2, frac3]])

            # Cross out fractions
            cross_lines = []
            for frac in [frac1, frac2, frac3]:
                line = Line(
                    frac.get_left(), frac.get_right(), color=RED, stroke_width=4
                )
                cross_lines.append(line)
            play_next(self, beats, *[Create(line) for line in cross_lines])
            ...
            play_text_next(self, beats, Write(definition))
