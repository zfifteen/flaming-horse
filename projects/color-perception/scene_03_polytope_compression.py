...
# Scene Class
class Scene03PolytopeCompression(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["compression"]) as tracker:
            # Title
            title = Text("Polytope Compression Ratio", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Equation
            equation = MathTex(
                r"\eta = \frac{\text{JNDs}}{k(k-1)}", font_size=48, color=GOLD
            )
            ...
            # Polytope Growth
            growth1 = Text("k=1: Dim=0", font_size=32)
            growth1.move_to(LEFT * 4 + DOWN * 1)
            growth2 = Text("k=2: Dim=2", font_size=32)
            growth2.next_to(growth1, DOWN, buff=0.3)
            safe_position(growth2)
            growth3 = Text("k=3: Dim=6", font_size=32)
            growth3.next_to(growth2, DOWN, buff=0.3)
            safe_position(growth3)
            play_in_slot(
                self,
                AnimationGroup(
                    FadeIn(growth1), FadeIn(growth2), FadeIn(growth3), lag_ratio=0.3
                ),
