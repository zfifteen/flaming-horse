...
class Scene03Step1(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["step1"]) as tracker:
            ...
            subtitle = Text("Divide, Multiply, Subtract", font_size=32, color=blues[1])
            ...
            # Setup: divisor, bracket, dividend (from previous scene context)
            ...
            setup = VGroup(divisor, bracket, dividend)
            play_next(self, beats, polished_fade_in(setup, lag_ratio=0.1), rate_func=smooth)
            ...
            # First quotient term
            quotient_term = MathTex(r"x^{2}", font_size=42, color=GREEN)
            quotient_term.next_to(bracket_top.get_left(), UP, buff=0.4, aligned_edge=LEFT)
            safe_position(quotient_term)
            play_next(self, beats, Write(quotient_term, run_time=1.5), rate_func=smooth)
            
            # Product under dividend
            product = MathTex(r"x^{3} + x^{2}", font_size=42, color=RED)
            product.next_to(dividend, DOWN, buff=0.15)
            product.align_to(dividend, LEFT)
            safe_position(product)
            play_next(self, beats, Write(product, run_time=2.0), rate_func=smooth)
            
            # Subtract animation with color change
            subtract_group = AnimationGroup(
                dividend.animate.set_color(YELLOW),
            ...
            # Partial remainder
            remainder = MathTex(r"2x^{2} + x - 3", font_size=42, color=blues[2])
            remainder.move_to(ORIGIN)
            play_next(self, beats, ReplacementTransform(dividend, remainder), FadeOut(product), rate_func=smooth)
            
            # Ensure no overlaps
            all_content = VGroup(title, subtitle, divisor, bracket, quotient_term, remainder)
