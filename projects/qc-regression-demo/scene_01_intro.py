    ...
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            # SLOT_START:scene_body
             beats = BeatPlan(tracker.duration, [2, 3, 2])

             palette = [BLUE, TEAL, GREEN]

             title = Text("QC Regression Pipeline Demo", font_size=48, weight=BOLD, color=palette[0])
             title.move_to(UP * 3.8)
             play_text_next(self, beats, Write(title))

             subtitle = Text("Introduction", font_size=36, color=palette[1])
             subtitle.next_to(title, DOWN, buff=0.4)
             safe_position(subtitle)
             play_text_next(self, beats, FadeIn(subtitle))

             data_box = VGroup(Rectangle(width=1.8, height=0.8, fill_opacity=0.3, stroke_width=3, color=palette[0]), Text("Data\\nOverview", font_size=24)).arrange(DOWN, buff=0.05)
             reg_box = VGroup(Rectangle(width=1.8, height=0.8, fill_opacity=0.3, stroke_width=3, color=palette[1]), Text("Regression\\nAnalysis", font_size=24)).arrange(DOWN, buff=0.05)
             res_box = VGroup(Rectangle(width=1.8, height=0.8, fill_opacity=0.3, stroke_width=3, color=palette[2]), Text("Results\\n& Metrics", font_size=24)).arrange(DOWN, buff=0.05)

             boxes = VGroup(data_box, reg_box, res_box).arrange(RIGHT, buff=0.6)
             arrow1 = Arrow(data_box.get_right(), reg_box.get_left(), buff=0.2, color=WHITE, stroke_width=4)
             arrow2 = Arrow(reg_box.get_right(), res_box.get_left(), buff=0.2, color=WHITE, stroke_width=4)
             arrows = VGroup(arrow1, arrow2)
             diagram = VGroup(boxes, arrows)
             diagram.move_to(DOWN * 1.2)
             safe_position(diagram)
             safe_layout(data_box, reg_box, res_box)

             play_next(self, beats, LaggedStart(Create(data_box), Create(reg_box), Create(res_box), Create(arrows), lag_ratio=0.15, rate_func=smooth))
