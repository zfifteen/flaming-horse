...
# Scene Class
class Scene03UncountableInfinity(VoiceoverScene):
    def construct(self):
        ...
        # 0.2 (title) + 0.3 (real line) + 0.5 (diagonalization) = 1.0 ✓
        with self.voiceover(text=SCRIPT["uncountable"]) as tracker:
            # Title
            title = Text("Uncountable Infinity", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Real number line
            number_line = NumberLine(
                x_range=[0, 1, 0.1], length=6, include_numbers=True
            )
            number_line.move_to(ORIGIN)
            play_in_slot(self, Create(number_line), tracker.duration * 0.3)

            # Diagonalization demonstration
            # List some decimals
            decimals = [
                "0.123456789...",
                "0.234567890...",
            ...
            decimal_texts = [Text(dec, font_size=24) for dec in decimals]
            for i, txt in enumerate(decimal_texts):
                txt.move_to(UP * (1 - i * 0.5) + LEFT * 3)
                safe_position(txt)
            # Add diagonal number
            diagonal = Text("0.456789012...", font_size=24, color=YELLOW)
            diagonal.move_to(DOWN * 1.5 + LEFT * 3)
            safe_position(diagonal)
            # Animate the list and diagonal
            play_in_slot(
                self,
                *[Write(txt) for txt in decimal_texts + [diagonal]],
