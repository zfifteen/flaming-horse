...
class Scene08Conclusion(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["conclusion"]) as tracker:
            # Title and summary
            title = Text("Conclusion and Implications", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            summary = Text(
                "η = B / D(k) bridges consciousness theory\nand sensory biology, revealing polytope compression\nand the geometry of experience",
                font_size=32,
                color=BLUE,
            )
            summary.next_to(title, DOWN, buff=0.5)
            safe_position(summary)
            group = VGroup(title, summary)
            play_text_in_slot(self, Write(group), tracker.duration * 0.3)

            # Fade out
            self.play(FadeOut(group), run_time=tracker.duration * 0.7)
