...
class Scene01Intro(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            # Title
            title = Text("The Paradox of Perception", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.3)

            # Subtitle
            subtitle = Text(
                "Why More Receptors Don't Always Mean Richer Experience", font_size=32
            )
            ...
            # Key Question
            question = Text(
                "Does more sensory input equal richer perception?",
                font_size=36,
                color=YELLOW,
            )
            question.move_to(ORIGIN)
            play_text_in_slot(self, Write(question), tracker.duration * 0.4)