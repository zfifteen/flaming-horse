...
class Scene01Intro(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["intro"]) as tracker:
            beats = BeatPlan(tracker.duration, [2, 3, 3])

            # Title
            title = Text("Understanding Irrational Numbers", font_size=48, weight=BOLD)
            ...
            play_next(self, beats, Create(number_line))

            # Highlight examples
            rational_label = Text("Rational", font_size=24, color=GREEN)
            rational_label.next_to(number_line.n2p(1), UP, buff=0.5)
            safe_position(rational_label)

            irrational_pi = MathTex(r"\pi", font_size=36, color=RED)
            irrational_pi.next_to(number_line.n2p(3.14), UP, buff=0.5)
            safe_position(irrational_pi)

            irrational_sqrt2 = MathTex(r"\sqrt{2}", font_size=36, color=RED)
            irrational_sqrt2.next_to(number_line.n2p(1.41), UP, buff=0.5)
            safe_position(irrational_sqrt2)

            examples = VGroup(rational_label, irrational_pi, irrational_sqrt2)
            play_next(self, beats, FadeIn(examples))
