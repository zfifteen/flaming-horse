...
class Scene02Data(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["data_overview"]) as tracker:
            # SLOT_START:scene_body
            beats = BeatPlan(tracker.duration, [1, 2, 2, 1])

            palette = [BLUE, TEAL, ORANGE]

            title = Text("Data Overview", font_size=48, weight=BOLD, color=palette[0])
            title.move_to(UP * 3.8)
            play_text_next(self, beats, Write(title))

            subtitle = Text("Sample QC Dataset", font_size=36, color=palette[1])
            subtitle.next_to(title, DOWN, buff=0.4)
            safe_position(subtitle)
            play_text_next(self, beats, FadeIn(subtitle))

            data_points = VGroup(
                Text("B1: Size 100, Defects 0.02 (01/01)", font_size=22),
                Text("B2: Size 150, Defects 0.03 (01/02)", font_size=22),
                Text("B3: Size 200, Defects 0.045 (01/03)", font_size=22),
                Text("B4: Size 250, Defects 0.065 (01/04)", font_size=22, color=RED),
                Text("B5: Size 300, Defects 0.04 (01/05)", font_size=22)
            ).arrange(DOWN, buff=0.15).scale(0.9).move_to(LEFT * 2.2)

            chart_values = [0.02, 0.03, 0.045, 0.065, 0.04]
            labels = VGroup(*[Text(f"B{i+1}", font_size=24) for i in range(5)]).arrange(DOWN, buff=0.25)
            bars = VGroup()
            for i, val in enumerate(chart_values):
                bar_height = val * 25
                bar = Rectangle(width=0.35, height=bar_height, fill_opacity=0.8, color=RED if val > 0.04 else ORANGE)
                bar.next_to(labels[i], RIGHT, buff=0.15)
                bar.shift(UP * bar_height / 2)
                bars.add(bar)
            bar_group = VGroup(labels, bars).move_to(RIGHT * 1.8)

            safe_position(data_points)
            safe_position(bar_group)
            safe_layout(data_points, bar_group)

            play_next(self, beats, LaggedStart(*[FadeIn(dp) for dp in data_points], lag_ratio=0.1))

            play_next(self, beats, LaggedStart(*[GrowFromCenter(bar) for bar in bars], lag_ratio=0.1, rate_func=smooth))

            highlight = SurroundingRectangle(bars[3], color=YELLOW, stroke_width=4)
            play_next(self, beats, ShowPassingFlash(highlight))
