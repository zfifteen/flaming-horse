...
# Scene Class
class Scene03GoodPractices(VoiceoverScene):
    def construct(self):
        # Qwen cached voiceover (precache required)
        self.set_speech_service(get_speech_service(Path(__file__).resolve().parent))

        # Animation Sequence
        # Timing budget: 0.2 + 0.25 + 0.35 + 0.1 + 0.1 = 1.0 ✓
        with self.voiceover(text=SCRIPT["good_practices"]) as tracker:
            # Title
            title = Text("Good Commit Message Practices", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.2)

            # Subject line example
            subject_label = Text(
                "Subject Line (< 50 chars):", font_size=32, color=YELLOW
            )
            subject_label.next_to(title, DOWN, buff=0.8)
            safe_position(subject_label)
            subject_example = Text(
                "Fix authentication timeout bug", font_size=36, color=GREEN
            )
            subject_example.next_to(subject_label, DOWN, buff=0.3)
            safe_position(subject_example)
            play_text_in_slot(
                self,
                *[Write(subject_label), Write(subject_example)],
                tracker.duration * 0.25,
            )

            # Body example with bullets
            body_label = Text("Body (optional, detailed):", font_size=32, color=YELLOW)
            body_label.next_to(subject_example, DOWN, buff=0.8)
            safe_position(body_label)
            bullets = VGroup(
                Text("• Explain what changed and why", font_size=24),
                Text("• Use bullet points for clarity", font_size=24),
                Text("• Keep concise but informative", font_size=24),
            ).arrange(DOWN, buff=0.3, aligned_edge=LEFT)
            bullets.next_to(body_label, DOWN, buff=0.3)
            safe_position(bullets)
            play_text_in_slot(
                self, FadeIn(bullets, lag_ratio=0.2), tracker.duration * 0.35
            )

            # Highlight key elements
            play_in_slot(
                self,
                Indicate(subject_example),
                tracker.duration * 0.1,
                max_run_time=1.0,
            )

            # Buffer to finish narration cleanly
