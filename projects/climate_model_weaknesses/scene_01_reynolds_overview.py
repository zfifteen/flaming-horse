...
# ── Scene Class ────────────────────────────────────────────────────
class Scene01ReynoldsOverview(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["reynolds_overview"]) as tracker:
            # Title
            title = Text("Reynolds Number Foundations", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            self.play(Write(title), run_time=tracker.duration * 0.4)

            # Subtitle with safe positioning
            subtitle = Text("From Bacteria to Hurricanes", font_size=32)
