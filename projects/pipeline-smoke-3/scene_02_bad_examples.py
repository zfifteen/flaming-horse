...
# Scene Class
class Scene02BadExamples(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["bad_examples"]) as tracker:
            # Title
            title = Text("Bad Commit Messages", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_in_slot(self, Write(title), tracker.duration * 0.15)

            # First bad example
            bad1 = Text("'fix bug'", font_size=36, color=RED)
            bad1.move_to(UP * 1)
            play_text_in_slot(self, FadeIn(bad1), tracker.duration * 0.12)
            cross1 = Cross(bad1, stroke_width=8, color=RED)
            play_in_slot(self, Create(cross1), tracker.duration * 0.13)

            # Second bad example
            bad2 = Text("'update code'", font_size=36, color=RED)
            bad2.move_to(ORIGIN)
            play_text_in_slot(self, FadeIn(bad2), tracker.duration * 0.12)
            cross2 = Cross(bad2, stroke_width=8, color=RED)
            play_in_slot(self, Create(cross2), tracker.duration * 0.13)

            # Third bad example
            bad3 = Text("'merge branch'", font_size=36, color=RED)
            bad3.move_to(DOWN * 1)
            play_text_in_slot(self, FadeIn(bad3), tracker.duration * 0.12)
            cross3 = Cross(bad3, stroke_width=8, color=RED)
            play_in_slot(self, Create(cross3), tracker.duration * 0.13)

            # Wait for conclusion narration
