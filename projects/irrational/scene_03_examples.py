...
# Scene Class
class Scene03Examples(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text=SCRIPT["examples"]) as tracker:
            beats = BeatPlan(tracker.duration, [4, 3, 3, 3])

            # Title
            title = Text("Famous Irrational Numbers", font_size=48, weight=BOLD)
            title.move_to(UP * 3.8)
            play_text_next(self, beats, Write(title))

            # Pi example
            pi_symbol = MathTex(r"\pi \approx 3.1415926535\ldots", font_size=36)
            pi_symbol.move_to(UP * 1.5)
            play_text_next(self, beats, Write(pi_symbol))

            # E example
            e_symbol = MathTex(r"e \approx 2.7182818284\ldots", font_size=36)
            e_symbol.move_to(ORIGIN)
            play_text_next(self, beats, Write(e_symbol))

            # Sqrt(2) example
            sqrt2_symbol = MathTex(r"\sqrt{2} \approx 1.4142135623\ldots", font_size=36)
            sqrt2_symbol.move_to(DOWN * 1.5)
            play_text_next(self, beats, Write(sqrt2_symbol))
