...
def main() -> int:
    ...
    parser.add_argument(
        "--scene-file",
        type=Path,
        help="Scene file path (for scene_repair phase)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Test mode - don't actually call API"
    )
    ...
    if not args.project_dir.exists():
        print(f"❌ Project directory does not exist: {args.project_dir}", file=sys.stderr)
        return 1
